...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    for (int i = 0; i < n_physical_pages; i++) {
        ...
        if (leastAT > ipt[i].last_access_time && ipt[i].last_access_time != -1) {
            leastAT = ipt[i].last_access_time;
        }
    }

    // 3) The virtual page is not in a physical page, lru
    //    and there is no free physical page
    //
    for (int i = 0; i < n_physical_pages; i++) {
        if (ipt[i].last_access_time == leastAT) {
